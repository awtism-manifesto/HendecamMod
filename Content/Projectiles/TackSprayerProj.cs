...
public class TackSprayerProj : ModProjectile
{
    ...
    {
        const int ShootFrequency = 16; // How long the sentry waits between shots.
        const int TargetingRange = 25 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 9.75f; // The velocity the sentry's shot projectile will travel.

        // Code to run when spawned
        if (JustSpawned)
        {
            JustSpawned = false;
            ShootTimer = ShootFrequency * 2.35f; // Delay the first shot slightly
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    int type = ModContent.ProjectileType<TackSpraySpawn>();
