...
public class MoltenSpear : ModProjectile
{
    ...
    protected virtual float HoldoutRangeMax => 198f;
    ...
    public override bool PreAI()
    {
        ...
        if (!Main.dedServ)
        {
            ...
            {
                Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Torch, Projectile.velocity.X * 2f, Projectile.velocity.Y * 2f, Alpha: 1, Scale: 1.4f);
            }

            if (Main.rand.NextBool(4))
            {
                Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Torch, Alpha: 1, Scale: 1.2f);
            }
        }
