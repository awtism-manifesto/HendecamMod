...
public class TrueFogSmoke : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 10, Projectile.height - 10, DustID.CursedTorch, 0f, 0f, 100, default, 1.5f);
            fireDust.fadeIn = 0.1f + Main.rand.Next(2) * 0.1f;
            fireDust.noGravity = true;
            fireDust.velocity *= 1.25f;
            Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 10, Projectile.height - 10, DustID.ShadowbeamStaff, 0f, 0f, 100, default, 1.5f);
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
            ...
            Vector2 Pos = target.Center - new Vector2(255, 0);

            // Pass "upwards = true" using ai[0] = 1f
            int spawnedProjectile = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Pos, velocity,
                ModContent.ProjectileType<TrueFogSmoke2>(), (int)(Projectile.damage * 0.5f), Projectile.knockBack, Projectile.owner);
            ...
            Vector2 Pos2 = target.Center - new Vector2(-255, 0);
