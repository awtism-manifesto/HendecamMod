...
public class CandyCaneShard : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (tick % 2 == 0)
            {
                Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 11, Projectile.height - 11, DustID.SnowBlock, 0f, 0f, 100, default, 0.8f);
                fire2Dust.fadeIn = 0.1f + Main.rand.Next(2) * 0.1f;
                fire2Dust.velocity *= 0.25f;
            }
            else
            {
                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 11, Projectile.height - 11, DustID.SnowBlock, 0f, 0f, 100, Color.Red, 0.8f);
