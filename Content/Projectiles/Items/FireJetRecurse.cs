...
public class FireJetRecurse : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile.alpha < 182)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 1, Projectile.height - 1, DustID.Torch, 0f, 0f, 100, default, 1.85f);
                fireDust.fadeIn = 0.3f + Main.rand.Next(5) * 0.2f;
                fireDust.noGravity = true;
                fireDust.velocity *= 1.5f;
            }
        }
    }
