...
public class BloodshotBomb : ModProjectile
{
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (int i = 0; i < 14; i++)
        {
            Dust smokeDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Blood, 0f, 0f, 100, default, 3.05f);
            smokeDust.velocity *= 9.5f;
            smokeDust.noGravity = true;
            Dust smoke3Dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Blood, 0f, 0f, 100, default, 1.75f);
            smoke3Dust.velocity *= 8.25f;
        }

        // Spawn a bunch of fire dusts.
        for (int j = 0; j < 19; j++)
        {
            Dust fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.RedTorch, 0f, 0f, 100, default, 3.5f);
