...
public class PlutoShotMini : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            Dust chudDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 3, Projectile.height - 3, DustType<PlutoniumDust>(), 0f, 0f, 100, default, 0.2f);
            chudDust.fadeIn = 0.2f + Main.rand.Next(5) * 0.1f;
            chudDust.velocity *= 0.05f;
        }

        float maxDetectRadius = 750f; // The maximum radius at which a projectile can detect a target
