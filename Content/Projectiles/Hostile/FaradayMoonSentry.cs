...
public class FaradayMoonSentry : ModProjectile
{
    ...
    // Constants for orbit
    private const float OrbitDistance = 185f;
    private const float FullRevolutionTime = 1f;

    // Constants for targeting and shooting
    private const int TargetingRange = 55 * 16;
    private const float FireVelocity = 15f;

    // Eclipse constants
    private const int EclipseCooldownFrames = 225; // Cooldown in frames (roughly 3.75 seconds at 60fps)
    ...
    public override void AI()
    {
        ...
        if (JustSpawned)
        {
            ...
            // Calculate orbit speed: 2p radians per 60 frames (1 second)
            OrbitSpeed = MathHelper.TwoPi / 90f;
        ...
        // Decrement eclipse cooldown
        if (EclipseCooldown > 0)
            EclipseCooldown--;

        // Find the sun sentry (if it exists)
        Projectile sunSentry = FindSunSentry();

        // Check if sun and moon are aligned (within angle tolerance)
        if (sunSentry != null && EclipseCooldown <= 0)
        {
            float sunAngle = sunSentry.ai[0]; // Sun's orbit angle is stored in ai[0]
            float angleDifference = Math.Abs(OrbitAngle - sunAngle);

            // Account for circular angle wrap-around
            angleDifference = Math.Min(angleDifference, MathHelper.TwoPi - angleDifference);

            // If angles are close enough, trigger eclipse
            if (angleDifference <= AngleTolerance)
            {
                // Spawn the eclipse laser
                if (Main.myPlayer == Projectile.owner)
                {
                    // Calculate direction to target (prioritize minion target or find an enemy)
                    ...
                    // Check minion attack target first
                    ...
                    {
                        // If no target, shoot outward from the player
                    ...
                    Projectile.NewProjectile(
                        ...
                        laserVelocity,
                        type,
                        (int)(Projectile.damage * 12.5), // High damage for the eclipse laser
                        5f,
                        Projectile.owner
                    );

                    // Play a powerful sound for the eclipse
                    ...
                    // Visual effects for the eclipse
                    for (int i = 0; i < 50; i++)
                    {
    ...
    private Projectile FindSunSentry()
    {
