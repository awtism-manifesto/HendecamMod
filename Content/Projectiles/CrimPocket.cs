        ...
        public override void AI()
        {
            ...
            if (Projectile.alpha < 222)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 3, Projectile.height - 3, DustID.Blood, 0f, 0f, 100, default, 0.55f);
                    fire2Dust.fadeIn = 0.2f + Main.rand.Next(3) * 0.1f;
                    fire2Dust.noGravity = true;
                    fire2Dust.velocity *= 1f;

                    Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 16, Projectile.height - 16, DustID.Ash, 0f, 0f, 100, default, 1.05f);
