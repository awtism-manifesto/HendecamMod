...
public class RingOfFireProj : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        const int TargetingRange = 20 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 0f; // The velocity the sentry's shot projectile will travel.

        // Code to run when spawned
        if (JustSpawned)
        {
            JustSpawned = false;
            ShootTimer = ShootFrequency * 2.5f; // Delay the first shot slightly
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                Dust d = Dust.NewDustPerfect(Projectile.Center, DustID.Torch, speed * 4, Scale: 1.5f);
                d.noGravity = true;
            }
        }
        ...
        {
            Dust dust = Dust.NewDustDirect(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.Torch, Projectile.oldVelocity.X * 0.5f, Projectile.oldVelocity.Y * 0.5f);
            dust.noGravity = true;
            dust.velocity *= 1.5f;
        }
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                // Play a shoot sound
                SoundEngine.PlaySound(SoundID.Item45 with { Volume = 2f }, Projectile.Center);
