...
public class WhippetProj : ModProjectile
{
    ...
    {
        target.AddBuff(BuffType<LilTMan>(), 240);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        for (int i = 0; i < list.Count - 1; i++)
        {
            ...
            Rectangle frame = new Rectangle(0, 0, 12, 18); // The size of the Handle (measured in pixels)
            ...
            if (i == list.Count - 2)
            {
                ...
                frame.Height = 100; // Height of the frame.

                // For a more impactful look, this scales the tip of the whip up when fully extended, and down when curled up.
                Projectile.GetWhipSettings(Projectile, out float timeToFlyOut, out int _, out float _);
                float t = Timer / timeToFlyOut;
                scale = MathHelper.Lerp(0.1f, 0.2f, Utils.GetLerpValue(0.2f, 0.3f, t, true) * Utils.GetLerpValue(0.4f, 0.5f, t, true));
            }
            else if (i > 10)
            {
                // Third segment
