...
public class ValiantYoyo : ModProjectile
{
    public override void SetStaticDefaults()
    {
        ...
        ProjectileID.Sets.YoyosMaximumRange[Projectile.type] = 333f;
    ...
    public override void AI()
    {
        ...
        if (Projectile.alpha < 187)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust kms = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.YellowTorch, 0f, 0f, 100, default, 0.55f);
                kms.fadeIn = 0.1f + Main.rand.Next(2) * 0.1f;
                kms.velocity *= 0.25f;

                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.Gastropod, 0f, 0f, 100, Color.Pink, 0.35f);
                fireDust.fadeIn = 0.1f + Main.rand.Next(2) * 0.1f;
                fireDust.velocity *= 0.2f;
                Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.Terragrim, 0f, 0f, 100, Color.LightSkyBlue, 1.35f);
