...
public class BullshitSwing : ModProjectile
{
    ...
    public override void SetDefaults()
    {
        ...
        Projectile.ownerHitCheckDistance = 363f; // The maximum range that the projectile can hit a target. 300 pixels is 18.75 tiles.
    ...
    public override void AI()
    {
        ...
        float scaleMulti = 2.5f; // Excalibur, Terra Blade, and The Horseman's Blade is 0.6f; True Excalibur is 1f; default is 0.2f 
        float scaleAdder = 2.66f; // Excalibur, Terra Blade, and The Horseman's Blade is 1f; True Excalibur is 1.2f; default is 1f 
        ...
        Vector2 dustPosition = Projectile.Center + dustRotation.ToRotationVector2() * 130f * Projectile.scale;
        Vector2 dustVelocity = (dustRotation + Projectile.ai[0] * MathHelper.PiOver2).ToRotationVector2();
        if (Main.rand.NextFloat() * 2f < Projectile.Opacity)
        {
            // Original Excalibur color: Color.Gold, Color.White
            Color dustColor = Color.Lerp(Color.Orange, Color.Orange, Main.rand.NextFloat() * 0.3f);
    ...
    public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
        ...
        float coneLength = 72f * Projectile.scale;
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        target.AddBuff(BuffID.Oiled, 444);
        target.AddBuff(BuffID.Frostburn, 444);
        target.AddBuff(BuffID.Frostburn2, 444);
        target.AddBuff(BuffID.CursedInferno, 444);
        target.AddBuff(BuffID.Ichor, 444);
        target.AddBuff(BuffID.Poisoned, 444);
        target.AddBuff(BuffID.Slimed, 444);
        target.AddBuff(BuffID.ShadowFlame, 444);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.DarkOrange * 0.6f * lerpTime, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, scale, spriteEffects, 0f);
        // Thin middle line (final frame)
        Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.DarkOrange * 0.5f * lerpTime, Projectile.rotation + Projectile.ai[0] * -0.05f, origin, scale * 0.8f, spriteEffects, 0f);
        // Thin bottom line (final frame)
        Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.DarkOrange * 0.4f * lerpTime, Projectile.rotation + Projectile.ai[0] * -0.1f, origin, scale * 0.6f, spriteEffects, 0f);
        ...
        for (float i = 0f; i < 8f; i += 1f)
        {
            ...
            DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos, new Color(255, 221, 135, 0) * lerpTime * (i / 9f), middleMediumColor, percentageOfLife, 0f, 0.5f, 0.5f, 1f, edgeRotation, new Vector2(0f, Utils.Remap(percentageOfLife, 0f, 1f, 3f, 0f)) * scale, Vector2.One * scale);
        }
        ...
        DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos2, new Color(255, 231, 145, 0) * lerpTime * 0.5f, middleMediumColor, percentageOfLife, 0f, 0.5f, 0.5f, 1f, 0f, new Vector2(2f, Utils.Remap(percentageOfLife, 0f, 1f, 4f, 1f)) * scale, Vector2.One * scale);
