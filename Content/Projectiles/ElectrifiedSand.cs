...
public class ElectrifiedSand : ModProjectile
{
    ...
    public override void SetDefaults()
    {
        ...
        Projectile.timeLeft = 490; // The live time for the projectile (60 = 1 second, so 600 is 10 seconds)
    ...
    public override void AI()
    {
        ...
        if (Projectile.alpha < 169)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 15, Projectile.height - 15, DustID.Sand, 0f, 0f, 100, default, 0.5f);
                fire2Dust.fadeIn = 0.1f + Main.rand.Next(1) * 0.1f;
                fire2Dust.noGravity = true;
                fire2Dust.velocity *= 1.05f;

                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 15, Projectile.height - 15, DustID.Electric, 0f, 0f, 100, default, 0.35f);
