...
public class TackShooterProj : ModProjectile
{
    ...
    {
        const int ShootFrequency = 24; // How long the sentry waits between shots.
        const int TargetingRange = 20 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 7.5f; // The velocity the sentry's shot projectile will travel.
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    int type = ModContent.ProjectileType<TackSpawn>();
