...
public class PyroDart : ModProjectile
{
        ...
        else
        {
            if (Projectile.alpha < 169)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 15, Projectile.height - 15, DustID.Torch, 0f, 0f, 100, default, 1.1f);
