...
public class AstaFission : ModProjectile
{
        ...
        else
        {
            if (Projectile.alpha < 169)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 15, Projectile.height - 15, DustType<AstatineDust>(), 0f, 0f, 100, default, 1.3f);
                    fire2Dust.fadeIn = 0.1f + Main.rand.Next(1) * 0.1f;
                    fire2Dust.noGravity = true;
                    fire2Dust.velocity *= 1.95f;

                    Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 15, Projectile.height - 15, DustType<AstatineDust>(), 0f, 0f, 100, default, 2.5f);
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (int i = 0; i < 35; i++)
        {
            Dust smokeDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<AstatineDust>(), 0f, 0f, 100, default, 1.5f);
        ...
        for (int j = 0; j < 50; j++)
        {
            Dust fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<AstatineDust>(), 0f, 0f, 100, default, 3.5f);
            fireDust.noGravity = true;
            fireDust.velocity *= 9f;
            fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<AstatineDust>(), 0f, 0f, 100, default, 1.5f);
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            Projectile.NewProjectile(
                ...
                position,
                velocity,
                ProjectileType<AstaBoomDelayed>(),
