...
public class OverdriveProj : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        const int ShootFrequency = 9; // How long the sentry waits between shots.
        const int TargetingRange = 30 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 12.15f; // The velocity the sentry's shot projectile will travel.

        // Code to run when spawned
        if (JustSpawned)
        {
            JustSpawned = false;
            ShootTimer = ShootFrequency * 5.95f; // Delay the first shot slightly
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    int type = ModContent.ProjectileType<OverSpawn>();
