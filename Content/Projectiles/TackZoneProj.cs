...
public class TackZoneProj : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        const int ShootFrequency = 5; // How long the sentry waits between shots.
        const int TargetingRange = 47 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 22.5f; // The velocity the sentry's shot projectile will travel.

        // Code to run when spawned
        if (JustSpawned)
        {
            JustSpawned = false;
            ShootTimer = ShootFrequency * 8.25f; // Delay the first shot slightly
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                // Play a shoot sound
                SoundEngine.PlaySound(SoundID.Item40 with { Volume = 1.33f }, Projectile.Center);
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    int type = ModContent.ProjectileType<ZoneSpawn>();
