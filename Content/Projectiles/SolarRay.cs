    ...
    public override void AI()
    {
        ...
        if (Projectile.alpha < 198)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust chudDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.HallowedWeapons, 0f, 0f, 100, default, 0.695f);
                chudDust.fadeIn = 0.1f + Main.rand.Next(5) * 0.1f;
                chudDust.velocity *= 0.25f;
                Dust kms = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.YellowTorch, 0f, 0f, 100, default, 0.695f);
                kms.fadeIn = 0.1f + Main.rand.Next(5) * 0.1f;
                kms.velocity *= 0.25f;

                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 6, Projectile.height - 6, DustID.Torch, 0f, 0f, 100, default, 0.375f);
