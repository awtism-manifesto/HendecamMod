...
/// </summary>
public class DemonBall : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile.alpha < 159)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust fire2Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 30, Projectile.height - 30, DustID.RedTorch, 0f, 0f, 100, default, 2.9f);
                fire2Dust.fadeIn = 0.2f + Main.rand.Next(5) * 0.1f;
                fire2Dust.noGravity = true;
                fire2Dust.velocity *= 3f;
                Dust fire4Dust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 20, Projectile.height - 20, DustID.Wraith, 0f, 0f, 100, default, 1.6f);
                fire4Dust.fadeIn = 0.2f + Main.rand.Next(2) * 0.1f;
                fire4Dust.noGravity = true;
                fire4Dust.velocity *= 1.3f;

                Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + 1f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.1f, Projectile.width - 30, Projectile.height - 30, DustID.DemonTorch, 0f, 0f, 100, default, 1.7f);
