...
public class HotShotsProj : ModProjectile
{
    ...
    {
        const int ShootFrequency = 17; // How long the sentry waits between shots.
        const int TargetingRange = 32 * 16; // The sentry's targeting range, 50 tiles.
        const float FireVelocity = 10.5f; // The velocity the sentry's shot projectile will travel.

        // Code to run when spawned
        if (JustSpawned)
        {
            JustSpawned = false;
            ShootTimer = ShootFrequency * 2.95f; // Delay the first shot slightly
        ...
        if (targetNPC != null)
        {
            if (ShootTimer <= 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    int type = ModContent.ProjectileType<HotTackSpawn>();
