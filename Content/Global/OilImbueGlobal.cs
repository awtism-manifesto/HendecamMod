...
namespace HendecamMod.Content.Global
{
    ...
    public class OilImbueGlobal : ModPlayer
    {
        ...
        public override void OnHitNPCWithItem(Item item, NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                target.AddBuff(BuffID.Oiled, 60 * Main.rand.Next(3, 7));
            }
        }
        ...
        public override void MeleeEffects(Item item, Rectangle hitbox)
        {
            if (exampleWeaponImbue && item.DamageType.CountsAsClass<MeleeDamageClass>() && !item.noMelee && !item.noUseGraphic)
            {
                ...
                {
                    Dust dust = Dust.NewDustDirect(new Vector2(hitbox.X, hitbox.Y), hitbox.Width, hitbox.Height, DustID.Wraith);
                    dust.velocity *= 0.5f;
                }
            }
        }
        ...
        public override void EmitEnchantmentVisualsAt(Projectile projectile, Vector2 boxPosition, int boxWidth, int boxHeight)
        {
            if (exampleWeaponImbue && (projectile.DamageType.CountsAsClass<MeleeDamageClass>() || ProjectileID.Sets.IsAWhip[projectile.type]) && !projectile.noEnchantments)
            {
                if (Main.rand.NextBool(5))
                {
                    Dust dust = Dust.NewDustDirect(boxPosition, boxWidth, boxHeight, DustID.Wraith);
                    dust.velocity *= 0.5f;
                }
            }
        }
