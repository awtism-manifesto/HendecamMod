...
public class KingslayerWarBow : ModItem
{
    public override void SetDefaults()
    {
        ...
        Item.shoot = ProjectileType<GelShotMini>();
    }

    public override void ModifyShootStats(Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback)
    {
        type = ProjectileType<GelShotMini>();
    }
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        for (int i = 0; i < 1; i++)
        {
            position = player.Center - new Vector2(Main.rand.NextFloat(133) * player.direction, 600f);
            position.Y -= 100 * i;
    ...
    public override void ModifyTooltips(List<TooltipLine> tooltips)
    {
        ...
        var line = new TooltipLine(Mod, "Face", "Shoots bouncy slime rain from the sky");
        tooltips.Add(line);
