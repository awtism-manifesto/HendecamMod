...
public class GlockAndBalls : ModItem
{
    public override void SetDefaults()
    {
        ...
        Item.scale = 0.8f;
        Item.rare = RarityType<HotPink>(); // The color that the item's name will be in-game.
        Item.value = 1450000;
    ...
    {
        type = ProjectileType<Projectiles.SpermRange>();
    }

    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        const int NumProjectiles = 1; // The number of projectiles that this gun will shoot.
        damage = (int)(damage * Main.rand.NextFloat(1.15f, 1.2f));
        for (int i = 0; i < NumProjectiles; i++)
        {
            // Rotate the velocity randomly by 30 degrees at max.
            Vector2 newVelocity = velocity.RotatedByRandom(MathHelper.ToRadians(3.1415f));
