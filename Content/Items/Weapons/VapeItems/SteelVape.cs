...
public class SteelVape : ModItem
{
    public override void SetDefaults()
    {
        ...
        Item.value = Item.sellPrice(silver: 111);
        ...
        Item.shoot = ProjectileType<GoldVapeSmoke>(); // ID of the projectiles the sword will shoot
        Item.shootSpeed = 8.15f; // Speed of the projectiles the sword will shoot
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        if (player.GetModPlayer<IronLungPlayer>().IronLungs == false)
        {
            for (int i = 0; i < NumProjectiles; i++)
            {
                ...
                Vector2 newVelocity = velocity.RotatedByRandom(MathHelper.ToRadians(5.7f));

                // Decrease velocity randomly for nicer visuals.
                newVelocity *= 1f - Main.rand.NextFloat(0.242f);
