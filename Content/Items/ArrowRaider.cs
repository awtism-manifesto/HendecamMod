...
public class ArrowRaider : ModItem
{
    public override void SetDefaults()
    {
        ...
        Item.damage = 91; // Sets the item's damage. Note that projectiles shot by this weapon will use its and the used ammunition's damage added together.
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        for (int i = 0; i < 1; i++)
        {
            position = player.Center - new Vector2(Main.rand.NextFloat(2) * player.direction, 600f);
            position.Y -= 100 * i;
    ...
    public override void ModifyTooltips(List<TooltipLine> tooltips)
    {
        ...
        var line = new TooltipLine(Mod, "Face", "Bombards your target with arrows from multiple angles");
        tooltips.Add(line);
