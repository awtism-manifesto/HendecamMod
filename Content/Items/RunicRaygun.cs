...
public class RunicRaygun : ModItem
{
    public override void SetDefaults()
    {
        ...
        Item.shoot = ModContent.ProjectileType<RuneShot>();

        Item.shootSpeed = 11.95f; // The speed of the projectile (measured in pixels per frame.)
    }

    public override void ModifyShootStats(Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback)
    {
        type = ModContent.ProjectileType<RuneShot>();
    }
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        for (int i = 0; i < NumProjectiles; i++)
        {
            ...
            Vector2 newVelocity = velocity.RotatedByRandom(MathHelper.ToRadians(0.0001f));

            // Decrease velocity randomly for nicer visuals.
            newVelocity *= 1f - Main.rand.NextFloat(0.0001f);
    ...
    public override void ModifyTooltips(List<TooltipLine> tooltips)
    {
        ...
        var line = new TooltipLine(Mod, "Face", "Shoots fast-moving rune shots that randomly deal between 1-5x base damage");
        tooltips.Add(line);
