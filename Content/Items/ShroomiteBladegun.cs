...
public class ShroomiteBladegun : ModItem
{
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        if (player.altFunctionUse == 2)
        {
            int NumProjectiles = 5; // The number of projectiles that this gun will shoot.
            damage = (int)(damage * Main.rand.NextFloat(0.66f, 0.67f));
            for (int i = 0; i < NumProjectiles; i++)
            {
                // Rotate the velocity randomly by 30 degrees at max.
                Vector2 newVelocity = velocity.RotatedByRandom(MathHelper.ToRadians(7.5f));
    ...
    public override void ModifyTooltips(List<TooltipLine> tooltips)
    {
        ...
        var line = new TooltipLine(Mod, "Face", "Left click to swing and release a duplicating slash of shroomite energy");
        tooltips.Add(line);
